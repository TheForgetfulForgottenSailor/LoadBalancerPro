...
- HEAD baseline: 9b5fac5e580b613c9c955dda97386a39e7ff9669
...
| Weighted Round Robin | Partially present | Legacy proportional `weightedDistribution(double)` exists. There is no request-level weighted round-robin strategy in `RoutingStrategyRegistry`. |
...
- `README.md` currently says `POST /api/routing/compare` supports `ROUND_ROBIN`, `TAIL_LATENCY_POWER_OF_TWO`, and `WEIGHTED_LEAST_LOAD`. That matches the current request-level registry/API surface.
...
3. Consider a narrow request-level weighted round-robin implementation only after the current registry/API behavior remains well characterized.
